...
is not guaranteed")
            ...
            x = solve_triangular(r,self.qb)
            self.permanentresidual = norm(self.A.dot(x)-self.b,2) ** 2
    ...
    """
    This function was only used for comparison. It is not part of LSSPAR, also it requires CVXPY and a solver that CVXPY supports. 
    ...
    def qr_lstsq(self,ind):
        ...
        res = norm(rhs[l:]) ** 2 + norm(self.qb[t:]) ** 2 + self.permanentresidual
    ...
    def qr_m(self):
        ...
        for i in range(self.n):
            ...
            p = l - v
            p = p/norm(p)
            H = eye(self.m-i) - 2*p.dot(p.T)
            A[i:,[i]] = w
            A[i:,i+1:] = H.dot(A[i:,i+1:])
            b[i:] = H.dot(b[i:])
        return [A,b]
    """
